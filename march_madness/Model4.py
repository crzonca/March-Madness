...
def get_team_ranking(graph, alpha=1):
    nodes = graph.nodes
    ...
    conference_mapping = TeamsInfo.conference_mapping()
...
def create_chance_df(conf_bt_df, use_all=True):
    teams = conf_bt_df.index if use_all else Bracket.get_teams_by_seed()
